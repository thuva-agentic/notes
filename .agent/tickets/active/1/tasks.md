...
- [x] Install `@tiptap/react`, `@tiptap/starter-kit`, and markdown shortcut extensions for WYSIWYG
- [x] Add `src/features/notes/components/NoteEditor.tsx` — Tiptap editor, debounced save via `notesApi.write`
- [x] Add `src/features/notes/components/NoteTitleInput.tsx` — editable title synced to note metadata
- [x] Wire "New note" action — create `.md` in selected folder, navigate to editor
- [x] Wire "Delete note" — confirm via `Dialog`, then `notesApi.delete` and refresh list
- [x] Persist `updatedAt` on each save; verify notes survive app restart

## Phase 8: Docs, acceptance & verification

- [x] Add `README.md` — prerequisites, `npm run dev`, project layout, app data location per OS
- [x] Update `.agent/techContext.md` and `.agent/systemPatterns.md` with confirmed stack and IPC channels
- [x] Verify acceptance: launch, CRUD, search (title/body/folder), sort (title default), nested folders, no network
- [x] Run `npm run lint` — fix any failures
- [x] Run `npm run build` — fix any failures
- [x] Run `npm test` (if configured) — domain unit tests pass
